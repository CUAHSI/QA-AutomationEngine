...
def post_datavalues(cursor, datavalues_csv, metadata_caches):
    for ind, datavalue_csv in enumerate(datavalues_csv, start=1):
        ...
        methodid = convert_to_id(cursor, metadata_caches, 'methods', 'MethodCode',
                                 datavalue_csv[8], 'MethodID')
        siteid = convert_to_id(cursor, metadata_caches, 'sites', 'SiteCode',
                               datavalue_csv[0], 'SiteID')
        sourceid = convert_to_id(cursor, metadata_caches, 'sources', 'SourceCode',
                                 datavalue_csv[9], 'SourceID')
        variableid = convert_to_id(cursor, metadata_caches, 'variables',
                                   'VariableCode', datavalue_csv[1], 'VariableID')
        ...
        if ind % batch_size == 1:
            datavalue_labels = ['DATAVALUE', 'LOCALDATETIME',
                                'UTCOFFSET', 'DATETIMEUTC',
...
def post_sites(cursor, sites_csv, metadata_caches):
    for ind, site_csv in enumerate(sites_csv, start=1):
        ...
        latlongdatumid = convert_to_id(cursor, metadata_caches, 'spatialreferences',
                                       'SRSName', site_csv[6], 'SpatialReferenceID')
...
def post_variables(cursor, variables_csv, metadata_caches):
    for ind, variable_csv in enumerate(variables_csv, start=1):
        ...
        variableunitsid = convert_to_id(cursor, metadata_caches, 'units',
                                        'UnitsName', variable_csv[2], 'UnitsID')
        timeunitsid = convert_to_id(cursor, metadata_caches, 'units', 'UnitsName',
