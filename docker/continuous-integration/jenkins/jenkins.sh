USERPASS=$(cat /var/jenkins_home/secrets/initialAdminPassword)
while read TEST; do
    CRUMB=$(curl -k -s "http://$JENKINSIP:8080/crumbIssuer/api/xml?xpath=concat(//crumbRequestField,\":\",//crumb)" --user "$USERNAME":"$USERPASS")
    if curl -s -X GET -k -H "$CRUMB" --user "$USERNAME":"$USERPASS" --output /dev/null --head --fail "http://$JENKINSIP:8080/job/$TESTSUITE-$TEST/lastBuild/api/xml?depth=1"; then
